...
export class DataService {
  ...
  getProducto(index:string){
    return this.http.get(`https://bd-tienda-3a77b.firebaseio.com/productos/${ index }.json`);
  }
  ...
  updateProductsStock(shoppingCartJSON){
    for(let i=0; i<shoppingCartJSON.length; i++){
      ...
      this.http.patch(`https://bd-tienda-3a77b.firebaseio.com/productos/${ index }.json`,JSON.stringify(data)).subscribe(()=>{
